...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  void _showSpotifyFeatureMessage() {
    showDialog(
      context: context,
      builder: (context) =>
          AlertDialog(
            title: Text(
                AppLocalizations.of(context).translate('new_feature')),
            content: Text(AppLocalizations.of(context).translate(
                'spotify_feature_message')),
            actions: [
              TextButton(
              ...
              ElevatedButton(
                ...
                child: Text(
                    AppLocalizations.of(context).translate('try_now')),
              ),
            ],
          ),
    );
  ...
  void _toggleOnlineMode() async {
    if (!_isOnlineMode) {
      bool authenticated = await _spotifyService.authenticate();
      if (authenticated) {
        setState(() {
          _isOnlineMode = true;
        });
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context).translate('spotify_auth_failed'))),
        );
      }
    } else {
      setState(() {
  ...
  Future<void> _fetchSpotifyData() async {
    try {
      final recentlyPlayedTracks = await _spotifyService.getRecentlyPlayedTracks();
      final spotifyPlaylists = await _spotifyService.getSpotifyCreatedPlaylists();
      setState(() {
        _recentlyPlayedTracks = recentlyPlayedTracks;
        _spotifyPlaylists = spotifyPlaylists;
      });
    } catch (e) {
      print('Error fetching Spotify data: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load Spotify data: $e')),
      );
    }
  }
  ...
  Widget buildHomeTab() {
    return RefreshIndicator(
      onRefresh: _isOnlineMode ? _fetchSpotifyData : _refreshLibrary,
        ...
        child: AnimationLimiter(
          child: Column(
            ...
            children: AnimationConfiguration.toStaggeredList(
              ...
              children: [
                buildOnlineModeToggle(),
                const SizedBox(height: 20.0),
                Text(
                  _isOnlineMode
                      ? AppLocalizations.of(context).translate('spotify_recommendations')
                      : AppLocalizations.of(context).translate('quick_access'),
                  style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white),
                ),
                const SizedBox(height: 10.0),
                _isOnlineMode ? buildSpotifyRecommendations() : buildQuickAccessSection(),
                const SizedBox(height: 30.0),
                Text(
                  _isOnlineMode
                      ? AppLocalizations.of(context).translate('your_spotify_playlists')
                      : AppLocalizations.of(context).translate('suggested_tracks'),
                  style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white),
                ),
                const SizedBox(height: 10.0),
                _isOnlineMode ? buildSpotifyPlaylists() : buildSuggestedTracksSection(),
                if (!_isOnlineMode) ...[
                  const SizedBox(height: 30.0),
                  Text(
  ...
  Widget buildOnlineModeToggle() {
    return Switch(
      value: _isOnlineMode,
      onChanged: (value) => _toggleOnlineMode(),
      activeColor: Colors.green,
    );
  }

  Widget buildSpotifyRecommendations() {
    if (_recentlyPlayedTracks.isEmpty) {
      return glassmorphicContainer(
        child: const Padding(
          padding: EdgeInsets.all(16.0),
          child: Text('No recommendations available', style: TextStyle(color: Colors.white)),
        ),
      );
    }
    ...
    return AnimationLimiter(
      child: Column(
        children: AnimationConfiguration.toStaggeredList(
          ...
          children: _recentlyPlayedTracks.map((track) {
            return Padding(
              ...
              child: glassmorphicContainer(
                child: ListTile(
                  ...
                  onTap: () async {
                    final audioPlayerService = Provider.of<AudioPlayerService>(context, listen: false);
                    try {
                      final trackDetails = await _spotifyService.getTrackDetails(track['id']);

                      if (trackDetails['preview_url'] != null) {
                        final spotifySong = SpotifySongModel(
                          id: track['id'],
                          title: track['name'],
                          artist: track['artists'][0]['name'],
                          album: track['album']['name'],
                          duration: track['duration_ms'],
                          uri: trackDetails['full_song_url'], // Replace with the actual key for the full song URL
                          artworkUrl: track['album']['images'][0]['url'],
                        );

                        audioPlayerService.setSpotifyPlaylist([spotifySong], 0);
                        await audioPlayerService.playSpotifySong();
                      } else {
                        throw Exception('Preview not available for this track');
                      }
                    } catch (e) {
                      print('Error playing track: $e');
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Unable to play this track: ${e.toString()}')),
                      );
                    }
                  },
                ),
                    ...
                    child: glassmorphicContainer(
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Image.network(playlist['images'][0]['url'], width: 80, height: 80),
                            const SizedBox(height: 8),
