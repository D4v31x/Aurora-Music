...
class _AnimatedMeshBackgroundState extends State<AnimatedMeshBackground>
  ...
  List<OVertex> _animateVertices(double animationValue) {
    ...
    for (int i = 0; i < _baseVertices.length; i++) {
      ...
      final xFrequency = 0.3 +
          random.nextDouble() * 0.3; // Moderate frequency for smoother movement
      final yFrequency = 0.3 +
          random.nextDouble() * 0.3; // Moderate frequency for smoother movement
      ...
      if (isCorner) {
        edgeFactor =
            0.3; // Minimal movement for corners but allow some movement for coverage
      } else if (colIndex == 0 ||
          colIndex == 2 ||
          rowIndex == 0 ||
          rowIndex == 2) {
      ...
      final amplitude =
          0.06 * edgeFactor * animationSpeed; // Use dynamic animation speed

      // Apply smooth movement pattern with controlled speed
      final xOffset =
          sin(animationValue * xFrequency + xPhaseOffset) * amplitude;
      final yOffset =
          cos(animationValue * yFrequency + yPhaseOffset) * amplitude;
      ...
      if (rowIndex == 1 && colIndex == 1) {
        ...
        final bezierAmount =
            0.05 + random.nextDouble() * 0.05; // Reduced bezier amount
        ...
        final bezierAmount =
            0.03 + random.nextDouble() * 0.03; // Very small bezier
