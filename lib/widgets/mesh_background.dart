...
class MeshBackground extends StatefulWidget {
  ...
  const MeshBackground({
    ...
    this.animated = true,
    this.animationDuration = const Duration(seconds: 3), // Much faster animation (was 5)
...
class _MeshBackgroundState extends State<MeshBackground> with SingleTickerProviderStateMixin {
  ...
  List<OVertex> _animateVertices(double animationValue) {
    ...
    for (int i = 0; i < _baseVertices.length; i++) {
      ...
      final amplitudeX = 0.15 + (sin(rowIndex + colIndex) * 0.08); // Doubled from 0.08 + 0.02
      final amplitudeY = 0.12 + (cos(rowIndex - colIndex) * 0.06); // Doubled from 0.06 + 0.02
      
      // Apply flowing wave motion with increased speed
      final flowTime = animationValue * widget.animationSpeed * 2.0; // Double the speed multiplier
      ...
      final secondaryX = sin(flowTime * baseFreqX * 1.7 + phaseX + 1.5) * amplitudeX * 0.6; // Increased from 0.3
      final secondaryY = cos(flowTime * baseFreqY * 1.3 + phaseY + 2.1) * amplitudeY * 0.6; // Increased from 0.3
      ...
      final bezierStrength = 0.08 + random.nextDouble() * 0.04; // Doubled from 0.04 + 0.02
      final bezierPhase = flowTime * 0.8 + (i * 0.3); // Increased phase speed from 0.5
      ...
      if (rowIndex > 0 && rowIndex < 3 && colIndex > 0 && colIndex < 3) {
        ...
        } else if (rowIndex == 3) { // Bottom edge
          animatedVertices.add(newVertex.bezier(
            north: OVertex(finalX, finalY - simpleCtrl),
          ));
        } else if (colIndex == 0) { // Left edge
          animatedVertices.add(newVertex.bezier(
            east: OVertex(finalX + simpleCtrl, finalY),
          ));
        } else if (colIndex == 3) { // Right edge
          animatedVertices.add(newVertex.bezier(
